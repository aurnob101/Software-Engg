...
export default function CheckPage() {
	...
	function validateEmail(email:string) {
	  ...
	  function validatePhoneNumber(phoneNumber:string) {
		return /^\d+$/.test(phoneNumber) && phoneNumber.length > 7
	  }
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const isValidPhone = validatePhoneNumber(data.phone)
		if(!isValidPhone){
			toast({
				title: "Please use valid phone"
			});
			return ;
		}
		...
		setIsLoading(false);
		router.push("/admin/makeSales/invoice/" + saleRef.id);
	};
	useEffect(()=>{
		if(!products.length)
		router.push('/admin/makeSales')
	},[])
	return (
